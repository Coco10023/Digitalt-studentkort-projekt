/* Lösningar av laboration 4, av Francisco De Leon 2025. */
...
function saveToHistory() { // Skapar en funktion som sparar historiken från input elementen
    ...
    let history = JSON.parse(localStorage.getItem("userInputs")) || []; /*Localstorage är en del av webb 
    ...
    history.push(inputs); // Lägger till datan från objektet på den tomma arrayen
    localStorage.setItem("userInputs", JSON-stringify(history)); /* setItem() sparar den nya arrayen i localStorage
...
function getHistory() { // Denna funktion hämtar hela historiken från localStorage så att användaren kan se den
    ...
    history.innerHTML = ""; // Tömmer historiken/listan från elementet så att det inte dupliceras varje gång det hämtas
